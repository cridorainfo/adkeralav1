...
/** Production cloud — baked into every packaged bus PC build. */
const PRODUCTION_CLOUD_URL = 'https://adkeralav1-production.up.railway.app';
...
function applyPackagedDefaults(app) {
  ...
  if (!process.env.ADKERALA_CLOUD_URL) {
    process.env.ADKERALA_CLOUD_URL =
      process.env.VITE_CLOUD_URL || PRODUCTION_CLOUD_URL;
  }
...
module.exports = { applyPackagedDefaults, PRODUCTION_CLOUD_URL };
